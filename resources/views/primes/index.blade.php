...
@section('title', 'Primes - MyGest')

@section('page-title', 'Primes')
...
<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:24px;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        <h2 style="font-size:16px;font-weight:600;color:#f1f5f9;margin:0;">Liste des primes</h2>
        <a href="{{ route('primes.create') }}" style="display:inline-block;padding:9px 18px;background:#2563eb;border-radius:8px;color:#fff;font-size:13px;font-weight:600;text-decoration:none;">Nouvelle prime</a>
    </div>
    ...
    <table style="border-collapse:collapse;width:100%;">
        ...
        <tbody>
            @forelse($primes as $prime)
                <tr>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">{{ $prime->employee->nom_complet }}</td>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">{{ $prime->motif }}</td>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">{{ number_format($prime->montant, 0, ',', ' ') }} FCFA</td>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">{{ $prime->mois }}</td>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">{{ $prime->annee }}</td>
                    <td style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);border-bottom:1px solid rgba(255,255,255,.04);">
                        <a href="{{ route('primes.show', $prime) }}" style="color:#60a5fa;text-decoration:none;font-size:12px;font-weight:500;margin-right:12px;">Voir</a>
                        <a href="{{ route('primes.edit', $prime) }}" style="color:#60a5fa;text-decoration:none;font-size:12px;font-weight:500;margin-right:12px;">Modifier</a>
                        <form action="{{ route('primes.destroy', $prime) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" style="padding:12px 14px;font-size:13px;color:rgba(255,255,255,.4);border-bottom:1px solid rgba(255,255,255,.04);text-align:center;">Aucune prime trouvée.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
