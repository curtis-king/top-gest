...
@section('title', 'Modifier écriture comptable - MyGest')

@section('page-title', 'Modifier écriture comptable')
...
<a href="{{ route('ecritures-comptables.show', $ecritureComptable) }}" style="color:#60a5fa;text-decoration:none;font-size:13px;font-weight:500;display:inline-flex;align-items:center;gap:6px;margin-bottom:20px;">&larr; Retour</a>
...
<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:24px;">
    <form method="POST" action="{{ route('ecritures-comptables.update', $ecritureComptable) }}" id="ecriture-form">
        @csrf
        @method('PUT')
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px;">
            <div>
                ...
                <select name="journal_comptable_id" class="mg-select" required>
                    ...
                    @foreach($journaux as $id => $libelle)
                        <option value="{{ $id }}" {{ old('journal_comptable_id', $ecritureComptable->journal_comptable_id) == $id ? 'selected' : '' }} style="color:#000;">{{ $libelle }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="date_ecriture" value="{{ old('date_ecriture', $ecritureComptable->date_ecriture ? \Carbon\Carbon::parse($ecritureComptable->date_ecriture)->format('Y-m-d') : '') }}" class="mg-input">
        ...
        <div style="margin-bottom:18px;">
            ...
            <input type="text" name="libelle" value="{{ old('libelle', $ecritureComptable->libelle) }}" class="mg-input">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:24px;">
            <div>
                ...
                <input type="text" name="reference" value="{{ old('reference', $ecritureComptable->reference) }}" class="mg-input">
            ...
            <div>
                ...
                <select name="agence_id" class="mg-select">
                    ...
                    @foreach($agences as $id => $nom)
                        <option value="{{ $id }}" {{ old('agence_id', $ecritureComptable->agence_id) == $id ? 'selected' : '' }} style="color:#000;">{{ $nom }}</option>
                    @endforeach
                </select>
        ...
        <div style="display:flex;gap:12px;margin-top:24px;">
            ...
            <a href="{{ route('ecritures-comptables.show', $ecritureComptable) }}" style="padding:10px 24px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;color:rgba(255,255,255,.5);text-decoration:none;font-size:13px;font-weight:500;text-align:center;transition:all .25s ease;" onmouseover="this.style.borderColor='rgba(255,255,255,.15)'" onmouseout="this.style.borderColor='rgba(255,255,255,.08)'">Annuler</a>
        </div>
    </form>
</div>
...
<script>
    ...
    const prefillLignes = @json($ecritureComptable->lignes->map(fn($l) => [
