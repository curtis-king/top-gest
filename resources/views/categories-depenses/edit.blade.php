...
@section('title', 'Modifier catégorie de dépense - MyGest')

@section('page-title', 'Modifier catégorie de dépense')

@section('content')
<div style="max-width:700px;">
    <a href="{{ route('categories-depenses.index') }}" style="color:#60a5fa;text-decoration:none;font-size:13px;font-weight:500;display:inline-flex;align-items:center;gap:6px;margin-bottom:20px;">&larr; Retour</a>

    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:24px;">
        <form method="POST" action="{{ route('categories-depenses.update', $categorieDepense) }}">
            @csrf
            @method('PUT')

            <div style="margin-bottom:18px;">
                <label style="font-size:13px;font-weight:500;color:rgba(255,255,255,.7);display:block;margin-bottom:6px;">Libellé</label>
                <input type="text" name="libelle" value="{{ old('libelle', $categorieDepense->libelle) }}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                @error('libelle') <div style="color:#f87171;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
            </div>

            <div style="margin-bottom:18px;">
                <label style="font-size:13px;font-weight:500;color:rgba(255,255,255,.7);display:block;margin-bottom:6px;">Compte de charge par défaut</label>
                <select name="compte_comptable_id" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                    <option value="" style="color:#000;">Sélectionner un compte</option>
                    @foreach($comptes as $id => $libelle)
                        <option value="{{ $id }}" {{ old('compte_comptable_id', $categorieDepense->compte_comptable_id) == $id ? 'selected' : '' }} style="color:#000;">{{ $libelle }}</option>
                    @endforeach
                </select>
                @error('compte_comptable_id') <div style="color:#f87171;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
            </div>
