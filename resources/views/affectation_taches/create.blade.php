...
@section('title', 'Nouvelle affectation - MyGest')
...
<div style="max-width:700px;">
    ...
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:24px;">
        <form method="POST" action="{{ route('affectation-taches.store') }}">
            @csrf
            ...
            <div style="margin-bottom:18px;">
                ...
                <select name="tache_project_id" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                    <option value="" style="color:#000;">Sélectionner une tâche</option>
                    @foreach($taches as $id => $name)
                        <option value="{{ $id }}" {{ old('tache_project_id') == $id ? 'selected' : '' }} style="color:#000;">{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div style="margin-bottom:18px;">
                ...
                <div style="display:flex;gap:16px;margin-bottom:12px;">
                    <label style="font-size:13px;color:rgba(255,255,255,.65);display:flex;align-items:center;gap:6px;cursor:pointer;">
                        <input type="radio" name="type_affectation" value="interne" {{ old('type_affectation', 'interne') == 'interne' ? 'checked' : '' }} onchange="document.getElementById('employee-field').style.display='block';document.getElementById('externe-field').style.display='none'">
                        Employé (interne)
                    </label>
                    <label style="font-size:13px;color:rgba(255,255,255,.65);display:flex;align-items:center;gap:6px;cursor:pointer;">
                        <input type="radio" name="type_affectation" value="externe" {{ old('type_affectation') == 'externe' ? 'checked' : '' }} onchange="document.getElementById('employee-field').style.display='none';document.getElementById('externe-field').style.display='block'">
                        Externe
                    </label>
                </div>
            </div>

            <div id="employee-field" style="margin-bottom:18px;display:{{ old('type_affectation', 'interne') == 'interne' ? 'block' : 'none' }};">
                ...
                <select name="employee_id" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                    ...
                    @foreach($employees as $id => $name)
                        <option value="{{ $id }}" {{ old('employee_id') == $id ? 'selected' : '' }} style="color:#000;">{{ $name }}</option>
                    @endforeach
                </select>
                @error('employee_id') <div style="color:#f87171;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
            </div>

            <div id="externe-field" style="margin-bottom:18px;display:{{ old('type_affectation') == 'externe' ? 'block' : 'none' }};">
                ...
                <input type="text" name="nom_complet" value="{{ old('nom_complet') }}" placeholder="Nom de l'externe ou raison sociale" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
