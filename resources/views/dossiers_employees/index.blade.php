...
@section('title', 'Dossiers Employés - MyGest')

@section('page-title', 'Dossiers employés')

@section('content')
    <div style="margin-bottom:20px;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px;">
            <h2 style="font-size:18px;font-weight:600;color:#f1f5f9;">Liste des dossiers</h2>
            <a href="{{ route('dossiers-employees.create') }}" class="btn-primary" style="text-decoration:none;">+ Nouveau dossier</a>
        </div>
        ...
        <form method="GET" action="{{ route('dossiers-employees.index') }}" style="display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap;">
            <div style="flex:1;min-width:200px;position:relative;">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher par nom d'employé..." style="width:100%;padding:10px 14px 10px 40px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6';this.style.boxShadow='0 0 0 3px rgba(59,130,246,.12)'" onblur="this.style.borderColor='rgba(255,255,255,.08)';this.style.boxShadow='none'">
            </div>
            <select name="sort" style="padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                ...
                <option value="employee_id" {{ request('sort') == 'employee_id' ? 'selected' : '' }} style="color:#000;">Employé</option>
                <option value="date_engagement" {{ request('sort') == 'date_engagement' ? 'selected' : '' }} style="color:#000;">Date d'engagement</option>
                <option value="type_contrat" {{ request('sort') == 'type_contrat' ? 'selected' : '' }} style="color:#000;">Type de contrat</option>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;">
            @forelse($dossiers as $dossier)
                @php
                    $empName = $dossier->employee?->nom_complet ?? '—';
                    $parts = explode(' ', $empName);
                    $initials = strtoupper(substr($parts[0] ?? '', 0, 1) . substr($parts[1] ?? '', 0, 1));
                    $statusValue = $dossier->status?->value ?? 'inconnu';
                    $statusColors = [
                        'actif' => '#4ade80',
                        'inactif' => '#f87171',
                        'suspendu' => '#facc15',
                    ...
                    $color = $statusColors[$statusValue] ?? 'rgba(255,255,255,.45)';
                @endphp
                <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:20px;transition:all .2s;position:relative;" onmouseover="this.style.borderColor='rgba(59,130,246,.3)';this.style.background='rgba(255,255,255,.05)'" onmouseout="this.style.borderColor='rgba(255,255,255,.06)';this.style.background='rgba(255,255,255,.03)'">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:14px;">
                        <div style="display:flex;align-items:center;gap:12px;">
                            <div style="width:42px;height:42px;border-radius:42px;background:linear-gradient(135deg,#8b5cf6,#6d28d9);display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:600;color:#fff;flex-shrink:0;">
                                {{ $initials }}
                            </div>
                            <div>
                                <div style="font-size:15px;font-weight:600;color:#f1f5f9;">{{ $empName }}</div>
                                <div style="font-size:12px;color:rgba(255,255,255,.35);">Dossier #{{ $dossier->id }}</div>
                            </div>
                        </div>
                        <div style="display:flex;gap:4px;">
                            <a href="{{ route('dossiers-employees.show', $dossier) }}" style="width:32px;height:32px;border-radius:8px;background:rgba(96,165,250,.1);border:1px solid rgba(96,165,250,.15);display:flex;align-items:center;justify-content:center;color:#60a5fa;text-decoration:none;transition:all .15s;" onmouseover="this.style.background='rgba(96,165,250,.2)'" onmouseout="this.style.background='rgba(96,165,250,.1)'" title="Voir">
                    ...
                    <div style="padding:12px 0;border-top:1px solid rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.04);display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                        <div>
                            <div style="font-size:10px;text-transform:uppercase;letter-spacing:.4px;color:rgba(255,255,255,.3);margin-bottom:2px;">Date d'engagement</div>
                            <div style="font-size:13px;color:rgba(255,255,255,.65);">{{ $dossier->date_engagement ? \Carbon\Carbon::parse($dossier->date_engagement)->format('d/m/Y') : '—' }}</div>
                        </div>
                        <div>
                            <div style="font-size:10px;text-transform:uppercase;letter-spacing:.4px;color:rgba(255,255,255,.3);margin-bottom:2px;">Type de contrat</div>
                            <div style="font-size:13px;color:rgba(255,255,255,.65);">{{ $dossier->type_contrat?->value ?? '—' }}</div>
                        </div>
                        <div>
                            ...
                            <div><span style="display:inline-block;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:500;background:{{ $color }}22;color:{{ $color }};">{{ $statusValue }}</span></div>
                        </div>
                        <div>
                            <div style="font-size:10px;text-transform:uppercase;letter-spacing:.4px;color:rgba(255,255,255,.3);margin-bottom:2px;">Date de fin</div>
                            <div style="font-size:13px;color:rgba(255,255,255,.65);">{{ $dossier->date_fin ? \Carbon\Carbon::parse($dossier->date_fin)->format('d/m/Y') : '—' }}</div>
                        </div>
                    </div>

                    <div style="padding-top:10px;display:flex;gap:8px;justify-content:flex-end;">
                        <a href="{{ route('dossiers-employees.edit', $dossier) }}" style="font-size:12px;font-weight:500;color:rgba(255,255,255,.45);text-decoration:none;padding:4px 10px;border-radius:6px;transition:all .15s;" onmouseover="this.style.color='#60a5fa';this.style.background='rgba(96,165,250,.08)'" onmouseout="this.style.color='rgba(255,255,255,.45)';this.style.background='transparent'">Modifier</a>
                        <form method="POST" action="{{ route('dossiers-employees.destroy', $dossier) }}" style="display:inline;" onsubmit="return confirm('Confirmer la suppression ?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <div style="grid-column:1/-1;padding:40px;text-align:center;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;">
                    <div style="font-size:14px;color:rgba(255,255,255,.3);">Aucun dossier trouvé.</div>
                </div>
            @endforelse
        </div>
