...
@section('title', 'Modifier paiement - MyGest')
...
<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:28px;">
    <form method="POST" action="{{ route('payements-employees.update', $payementEmployee) }}" id="payForm">
        @csrf
        @method('PUT')
            ...
            <div class="pay-field">
                ...
                <select name="employee_id" id="employee_id" class="pay-input" required onchange="onPayChange()">
                    ...
                    @foreach($employees as $id => $nom)
                        <option value="{{ $id }}" style="color:#000;" {{ old('employee_id', $payementEmployee->employee_id) == $id ? 'selected' : '' }}>{{ $nom }}</option>
                    @endforeach
                </select>
            ...
            <div class="pay-field">
                ...
                <select name="status" id="status" class="pay-input" required>
                    ...
                    @foreach($statuses as $s)
                        <option value="{{ $s->value }}" style="color:#000;" {{ old('status', $payementEmployee->status?->value ?? $payementEmployee->status) == $s->value ? 'selected' : '' }}>{{ $s->value }}</option>
                    @endforeach
                </select>
            ...
            <div class="pay-field">
                ...
                <select name="mois" id="mois" class="pay-input" required onchange="onPayChange()">
                    ...
                    @foreach($moisFr as $i => $m)
                        <option value="{{ $m }}" style="color:#000;" {{ old('mois', $payementEmployee->mois) == $m ? 'selected' : '' }}>{{ $m }}</option>
                    @endforeach
                </select>
            ...
            <div class="pay-field">
                ...
                <select name="annee" id="annee" class="pay-input" required onchange="onPayChange()">
                    @for($y = date('Y'); $y >= date('Y') - 5; $y--)
                        <option value="{{ $y }}" style="color:#000;" {{ old('annee', $payementEmployee->annee) == $y ? 'selected' : '' }}>{{ $y }}</option>
                    @endfor
                </select>
        ...
        <div class="pay-grid">
            <div class="pay-field">
                ...
                <input type="number" name="salaire_base" id="salaire_base" value="{{ old('salaire_base', $payementEmployee->salaire_base) }}" step="0.01" min="0" class="pay-input pay-readonly" placeholder="0" readonly>
                <span class="pay-auto" id="salaireInfo">Salaire de la fonction</span>
            ...
            <div class="pay-field">
                ...
                <input type="datetime-local" name="date_paiement" id="date_paiement" value="{{ old('date_paiement', $payementEmployee->date_paiement ? $payementEmployee->date_paiement->format('Y-m-d\TH:i') : '') }}" class="pay-input">
        ...
        <div class="pay-grid">
            <div class="pay-field">
                ...
                <input type="number" name="total_primes" id="total_primes" value="{{ old('total_primes', $payementEmployee->total_primes) }}" step="0.01" min="0" class="pay-input" placeholder="0" readonly style="color:#4ade80;cursor:default;">
            ...
            <div class="pay-field">
                ...
                <input type="number" name="total_retenus" id="total_retenus" value="{{ old('total_retenus', $payementEmployee->total_retenus) }}" step="0.01" min="0" class="pay-input" placeholder="0" readonly style="color:#f87171;cursor:default;">
                <span class="pay-auto">Calculé automatiquement</span>
        ...
        <div class="pay-summary">
            <div class="pay-summary-row">
                ...
                <span class="pay-summary-value" id="preview_base">{{ number_format($payementEmployee->salaire_base, 0, ',', ' ') }} FCFA</span>
            </div>
            <div class="pay-summary-row">
                <span class="pay-summary-label">+ Total primes</span>
                <span class="pay-summary-value" style="color:#4ade80;" id="preview_primes">{{ number_format($payementEmployee->total_primes ?? 0, 0, ',', ' ') }} FCFA</span>
            </div>
            <div class="pay-summary-row">
                <span class="pay-summary-label">- Total retenus</span>
                <span class="pay-summary-value" style="color:#f87171;" id="preview_retenus">{{ number_format($payementEmployee->total_retenus ?? 0, 0, ',', ' ') }} FCFA</span>
            </div>
            <div class="pay-summary-row">
                <span class="pay-summary-label">Net à payer</span>
                <span class="pay-summary-value net" id="preview_net">{{ number_format($payementEmployee->net_a_payer, 0, ',', ' ') }} FCFA</span>
            </div>
        </div>
        ...
        <input type="hidden" name="net_a_payer" id="net_a_payer" value="{{ old('net_a_payer', $payementEmployee->net_a_payer) }}">
