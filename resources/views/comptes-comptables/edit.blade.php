...
@section('title', 'Modifier compte comptable - MyGest')

@section('page-title', 'Modifier compte comptable')
...
<div style="max-width:700px;">
    ...
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:24px;">
        <form method="POST" action="{{ route('comptes-comptables.update', $compteComptable) }}">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom:18px;">
                ...
                @if($compteComptable->is_systeme)
                    <div style="width:100%;padding:10px 14px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:10px;font-size:13px;color:rgba(255,255,255,.6);">{{ $compteComptable->numero_compte }}</div>
                    <div style="font-size:12px;color:rgba(255,255,255,.35);margin-top:6px;">Compte standard du plan SYSCOHADA — libellé et statut modifiables uniquement.</div>
                @else
                    <input type="text" value="{{ $compteComptable->numero_compte }}" disabled style="width:100%;padding:10px 14px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:10px;font-size:13px;font-family:inherit;color:rgba(255,255,255,.4);outline:none;">
                @endif
            </div>

            <div style="margin-bottom:18px;">
                <label style="font-size:13px;font-weight:500;color:rgba(255,255,255,.7);display:block;margin-bottom:6px;">Libellé</label>
                <input type="text" name="libelle" value="{{ old('libelle', $compteComptable->libelle) }}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;transition:all .25s ease;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                @error('libelle') <div style="color:#f87171;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
            </div>

            @unless($compteComptable->is_systeme)
                <div style="margin-bottom:18px;">
                    ...
                    <select name="type_compte" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                        @foreach($types as $t)
                            <option value="{{ $t->value }}" {{ old('type_compte', $compteComptable->type_compte?->value) == $t->value ? 'selected' : '' }} style="color:#000;">{{ $t->value }}</option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <select name="sens_normal" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                        @foreach($sens as $s)
                            <option value="{{ $s->value }}" {{ old('sens_normal', $compteComptable->sens_normal?->value) == $s->value ? 'selected' : '' }} style="color:#000;">{{ $s->value }}</option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <select name="compte_parent_id" style="width:100%;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;font-size:13px;font-family:inherit;color:#fff;outline:none;cursor:pointer;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='rgba(255,255,255,.08)'">
                        ...
                        @foreach($comptesParents as $id => $numero)
                            <option value="{{ $id }}" {{ old('compte_parent_id', $compteComptable->compte_parent_id) == $id ? 'selected' : '' }} style="color:#000;">{{ $numero }}</option>
                        @endforeach
                    </select>
            ...
            <div style="margin-bottom:18px;">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                    <input type="checkbox" name="actif" value="1" {{ old('actif', $compteComptable->actif) ? 'checked' : '' }} style="accent-color:#3b82f6;">
                    <span style="font-size:13px;color:rgba(255,255,255,.7);">Actif</span>
                </label>
                @error('actif') <div style="color:#f87171;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
            </div>
